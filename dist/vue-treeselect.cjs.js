...
/* harmony default export */ var treeselectMixin = ({
  ...
  computed: {
    ...
    visibleOptionPaths: function visibleOptionPaths() {
      var _this3 = this;

      var visibleOptionPaths = [];
      this.traverseAllNodesByIndex(function (node) {
  ...
  methods: {
    ...
    getNodeByPath: function getNodeByPath(nodePath) {
      warning_warning(function () {
        return nodePath != null;
      }, function () {
        return "Invalid node path: ".concat(nodePath);
      });
      if (nodePath == null) return null;
      return nodePath in this.forest.nodePathMap ? this.forest.nodePathMap[nodePath] : null;
    },
    ...
    setCurrentHighlightedOption: function setCurrentHighlightedOption(node) {
      ...
      var prevPath = this.menu.current;

      if (prevPath != null) {
        var prev = this.getNodeByPath(prevPath);
      ...
      if (this.menu.isOpen && scroll) {
        var scrollToOption = function scrollToOption() {
          ...
          var $option = $menu.querySelector(".vue-treeselect__option[data-path=\"".concat(node.path, "\"]"));
    ...
    resetHighlightedOptionWhenNecessary: function resetHighlightedOptionWhenNecessary() {
      ...
      if (forceReset || current == null || !(current in this.forest.nodePathMap) || !this.shouldShowOptionInMenu(this.getNodeByPath(current))) {
        this.highlightFirstOption();
      }
    },
    highlightFirstOption: function highlightFirstOption() {
      if (!this.hasVisibleOptions) return;
      var first = this.visibleOptionPaths[0];
      this.setCurrentHighlightedOption(this.getNodeByPath(first));
    },
    highlightPrevOption: function highlightPrevOption() {
      if (!this.hasVisibleOptions) return;
      var prev = this.visibleOptionPaths.indexOf(this.menu.current) - 1;
      if (prev === -1) return this.highlightLastOption();
      this.setCurrentHighlightedOption(this.getNodeByPath(this.visibleOptionPaths[prev]));
    },
    highlightNextOption: function highlightNextOption() {
      if (!this.hasVisibleOptions) return;
      var next = this.visibleOptionPaths.indexOf(this.menu.current) + 1;
    ...
    highlightLastOption: function highlightLastOption() {
      if (!this.hasVisibleOptions) return;
      var last = last_default()(this.visibleOptionPaths);
    ...
    generatePath: function generatePath(node) {
      if (node.parentNode !== NO_PARENT_NODE) {
    ...
    normalize: function normalize(parentNode, nodes, prevNodeMap) {
      ...
      var normalizedOptions = nodes.map(function (node) {
        ...
        var path = _this17.generatePath(normalized);

        _this17.$set(normalized, 'path', _this17.generatePath(normalized));

        _this17.$set(_this17.forest.nodePathMap, path, normalized);
    ...
    checkDuplication: function checkDuplication(node) {
      ...
      if (this.disableBranchNodes) {
        warning_warning(function () {
          return !(node.path in _this20.forest.nodePathMap && !_this20.forest.nodePathMap[node.path].isFallbackNode);
        }, function () {
          return "Detected duplicate presence of node path ".concat(JSON.stringify(node.path), ". ") + "Their labels are \"".concat(_this20.forest.nodePathMap[node.path].label, "\" and \"").concat(node.label, "\" respectively.");
        });
      } else {
        warning_warning(function () {
          return !(node.id in _this20.forest.nodeMap && !_this20.forest.nodeMap[node.id].isFallbackNode);
        }, function () {
          return "Detected duplicate presence of node id ".concat(JSON.stringify(node.id), ". ") + "Their labels are \"".concat(_this20.forest.nodeMap[node.id].label, "\" and \"").concat(node.label, "\" respectively. ") + "Enable experimental node path support by disabling branch node selection if this is intentional.";
        });
      }
    },
    ...
    onKeyDown: function onKeyDown(evt) {
      ...
      switch (key) {
        ...
        case KEY_CODES.ENTER:
          {
            ...
            var current = instance.getNodeByPath(instance.menu.current);
        ...
        case KEY_CODES.ARROW_LEFT:
          {
            var _current = instance.getNodeByPath(instance.menu.current);
        ...
        case KEY_CODES.ARROW_RIGHT:
          {
            var _current2 = instance.getNodeByPath(instance.menu.current);
